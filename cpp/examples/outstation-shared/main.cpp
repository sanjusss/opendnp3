...
DatabaseConfig ConfigureDatabase()
{
    ...
    config.analog_input[0].defaultValue = Analog(15);
    return config;
}
...
int main(int argc, char* argv[])
{
    ...
    { // Specify what log levels to use. NORMAL is warning and above
        ...
        try
        {
            channel = manager.AddSharedTcpServer("server", logLevels, IPEndpoint("0.0.0.0", 20000),
                                                 PrintingChannelListener::Create(), nullptr);
        }
        ...
        while (true)
        {
            ...
            std::cout << "c = counter, b = binary, d = doublebit, a = analog, o = octet string, 'quit' = exit"
                      << std::endl;
            std::cin >> input;

            if (input == "quit")
                break; // DNP3Manager destructor cleanups up everything automatically
            else
            {
